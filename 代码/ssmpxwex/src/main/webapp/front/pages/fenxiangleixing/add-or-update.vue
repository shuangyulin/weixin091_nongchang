<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">分享类型</view>
				<input :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"110rpx"}' :disabled="ro.fenxiangleixing" v-model="ruleForm.fenxiangleixing" placeholder="分享类型"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`fenxiangleixing`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					if(o=='fenxiangleixing'){
					this.ruleForm.fenxiangleixing = obj[o];
					this.ro.fenxiangleixing = true;
					continue;
					}
					if(o=='tupian'){
		...
		methods: {
			...
			async onSubmitTap() {
																																																																																																																																																																								if(this.ruleForm.id){
					await this.$api.update(`fenxiangleixing`, this.ruleForm);
				}else{
					await this.$api.add(`fenxiangleixing`, this.ruleForm);
				}
